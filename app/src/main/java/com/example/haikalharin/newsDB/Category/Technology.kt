    ...
    fun refresh() {
        ...
        api!!.getListNews("us", category, BuildConfig.NEWS_API_TOKEN)?.enqueue(object : Callback<ResponseNews?> {
            override fun onResponse(call: Call<ResponseNews?>?, response: Response<ResponseNews?>?) {
                if (response != null) {
                    if (response.isSuccessful) {
                        ...
                        news!!.adapter = NewsAdapter(this@Technology, list)
                        adapter!!.notifyDataSetChanged()
                    } else {
                        hideDialog()
                        Toast.makeText(this@Technology, "Gagal mengambil data !", Toast.LENGTH_SHORT).show()
                    }
                }
            }
            ...
            override fun onFailure(call: Call<ResponseNews?>?, t: Throwable?) {
                hideDialog()
                Toast.makeText(this@Technology, "Gagal menyambung ke internet !", Toast.LENGTH_SHORT).show()
            }
        })
    }
